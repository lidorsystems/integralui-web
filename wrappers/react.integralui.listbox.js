/*
  filename: react.integralui.listbox.js
  version : 23.3.0
...
class IntegralUIListBoxComponent extends Component {
    constructor(props){
        super(props);
    ...
    _initProps(){
        if (this._isCtrlDefined()){
            ...
            if (this._isDefined(this.props.showGroups))
                this.ctrlRef.current.showGroups = this.props.showGroups;
	...
	async addItem(item){
        return this._isCtrlDefined() ? this.ctrlRef.current.addItem(item) : Promise.resolve();
	}

	async clearItems(){
        return this._isCtrlDefined() ? this.ctrlRef.current.clearItems() : Promise.resolve();
    }

	async insertItemAt(item, index){
        return this._isCtrlDefined() ? this.ctrlRef.current.insertItemAt(item, index) : Promise.resolve();
	}
	...
	async removeItemAt(index){
        return this._isCtrlDefined() ? this.ctrlRef.current.removeItemAt(index) : Promise.resolve();
	}
    ...
    async collapse(group){
        return this._isCtrlDefined() ? this.ctrlRef.current.collapse(group) : Promise.resolve();
    }

    async expand(group){
        return this._isCtrlDefined() ? this.ctrlRef.current.expand(group) : Promise.resolve();
    }

    async toggle(group, value){
        return this._isCtrlDefined() ? this.ctrlRef.current.toggle(group, value) : Promise.resolve();
    }
    ...
    render(){
        return (
            <iui-listbox id={this.props.id} ref={this.ctrlRef}>
                {this.props.children}
            </iui-listbox>
        );
    }
