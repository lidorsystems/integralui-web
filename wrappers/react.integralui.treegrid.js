/*
  filename: react.integralui.treegrid.js
  version : 23.3.0
...
class IntegralUITreeGridComponent extends Component {
    constructor(props){
        super(props);
    ...
    loadData(data, parent, fields, flat, incremental){
        if (this._isCtrlDefined())
            this.ctrlRef.current.loadData(data, parent, fields, flat, incremental);
    }
    ...
    render(){
        return (
            <iui-treegrid id={this.props.id} ref={this.ctrlRef}>
                {this.props.children}
            </iui-treegrid>
        );
    }
