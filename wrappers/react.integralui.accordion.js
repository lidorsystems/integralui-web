/*
  filename: react.integralui.accordion.js
  version : 23.3.0
...
class IntegralUIAccordionComponent extends Component {
    constructor(props){
        super(props);
	...
	collapse(group){
        if (this._isCtrlDefined())
            this.ctrlRef.current.collapse(group);
    }

	expand(group){
        if (this._isCtrlDefined())
            this.ctrlRef.current.expand(group);
    }
    ...
    render(){
        return (
            <iui-accordion id={this.props.id} ref={this.ctrlRef}>
                {this.props.children}
            </iui-accordion>
        );
    }
